...
import adventofcode.calendar.year2019.things.Wire;
...
public class Day3Solver implements Solver {
    ...
    public Day3Solver(String input) {
        ...
        if (lines.length != 2) {
            throw new IllegalArgumentException("wrong number of lines in input: " + lines.length);
        }
        A = Wire.parse(lines[0]);
        B = Wire.parse(lines[1]);
    }

    @Override
    public Object part1() {
        int minDistance = Integer.MAX_VALUE;
        for (Wire.Point point : A.intersect(B)) {
